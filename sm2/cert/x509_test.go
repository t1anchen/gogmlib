...
func TestCreateCertificateRequest(t *testing.T) {
	pri, pub, err := sm2.GenKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	...
	csrPub := csr.PublicKey.(*sm2.PubKey)
	if !bytes.Equal(pub.ToUncompressedBytes(), csrPub.ToUncompressedBytes()) {
...
func TestCreateCertificate(t *testing.T) {
	...
	sign, err := pri.Sign(nil, cinfo.Raw)
	if err != nil {
		t.Fatal(err)
	}
